...
func TestAnalyzeUseCase_Execute_PublishesDirectoryQualityHierarchy(t *testing.T) {
	projectRoot := t.TempDir()
	t.Chdir(projectRoot)

	sources := map[string]string{
		"root.py": `def root_value():
	return 1
`,
		filepath.Join("pkg", "hotspot.py"): `def hotspot(value):
	if value > 10:
		return value
	return 0
`,
		filepath.Join("pkg", "sub", "helper.py"): `def helper(value):
	return value
`,
	}
	for path, source := range sources {
		...
		if err := os.WriteFile(path, []byte(source), 0o644); err != nil {
			t.Fatalf("write Python source %q: %v", path, err)
		}
	}
	...
	directories := make(map[string]domain.DirectoryQualityMetrics, len(response.DirectoryQuality))
	for _, directory := range response.DirectoryQuality {
		directories[directory.DirectoryPath] = directory
	}

	root := directories["."]
	if root.ModuleCount != 3 || root.DirectModuleCount != 1 {
		t.Fatalf("expected root to contain 3 modules, 1 direct; got %+v", root)
	}
	pkg := directories["pkg"]
	if pkg.ModuleCount != 2 || pkg.DirectModuleCount != 1 {
		t.Fatalf("expected pkg to contain 2 modules, 1 direct; got %+v", pkg)
	}
	sub := directories[filepath.Join("pkg", "sub")]
	if sub.ModuleCount != 1 || sub.DirectModuleCount != 1 {
		t.Fatalf("expected pkg/sub to contain 1 direct module; got %+v", sub)
	}

	if root.AnalyzedFunctionCount != 3 || pkg.AnalyzedFunctionCount != 2 || sub.AnalyzedFunctionCount != 1 {
		t.Fatalf("expected recursive function rollups, got root=%+v pkg=%+v sub=%+v", root, pkg, sub)
	}
	if root.LinesOfCode <= pkg.LinesOfCode || pkg.LinesOfCode <= sub.LinesOfCode {
		t.Fatalf("expected recursive line rollups, got root=%d pkg=%d sub=%d", root.LinesOfCode, pkg.LinesOfCode, sub.LinesOfCode)
	}
}
