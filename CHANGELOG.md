...
- **BREAKING**: Narrow `check --allow-parse-errors` to syntax and parser failures. Missing, unreadable, and permission-denied files still fail the gate.
- **BREAKING**: Match bare file globs against basenames at every directory depth. For example, `main*.py` now also matches `pkg/main_utils.py`; add a directory segment to constrain the rule.
