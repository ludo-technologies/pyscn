...
`@staticmethod`、`@classmethod`、`@abstractmethod` で装飾されたメソッドはインスタンス状態を参照しないため、グラフから除外されます。

コンストラクタ（`__init__`、`__new__`、および dataclass の `__post_init__` フック）も除外されます。コンストラクタは通常クラスの全属性を初期化するため、グラフに残すと本来無関係な責務クラスタ同士が連結され、ほぼすべてのクラスで `LCOM4` が `1` に潰れてしまいます。コンストラクタが導入する属性は `instance_variables` には引き続き計上されます。
