...
| `0` | 問題なし | パス |
| `1` | 閾値を超える問題が見つかった | 失敗 |
| `2` | 分析を完了できなかった（不正な入力、存在しないファイル、パースできないファイル） | 失敗 |

終了コード `2` は、ゲートがコード全体を見られなかったことを意味します。たとえば構文エラーを含むファイルはすべての分析から除外されるため、検出結果を一切生成せず、放置すると合格してしまいます。パースできないファイルを警告として扱うには `--allow-parse-errors` を指定してください。
