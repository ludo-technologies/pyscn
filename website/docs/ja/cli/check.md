...
!!! note "パースエラーのカバー範囲"
    選択したすべての解析は同じプロジェクト検出台帳を使用します。パースエラーがある場合、`--allow-parse-errors` を指定しない限り、`pyscn check --select deps` を含めてチェックは失敗します。ファイル読み込みエラーは常に失敗となります。
...
| `--allow-parse-errors`   | off  | パースエラーを警告のみとします。ファイル読み込みエラーは引き続きチェックを失敗させます。 |

デフォルトでは、パースできないファイルがあるとチェックは失敗します。そのようなファイルはすべての解析から除外されるため検出結果を一切生成せず、放置するとすべてのしきい値を通過してしまいます。`--allow-parse-errors` は、存在しないファイル、権限エラー、その他の読み込みエラーを免除しません。
