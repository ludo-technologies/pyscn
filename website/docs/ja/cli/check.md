...
- パスすれば **exit 0**、問題が見つかれば **exit 1**、解析を完了できなければ **exit 2**。
- **厳格なデフォルト** — 複雑度 10 を超える関数があれば失敗、循環依存があれば失敗（`--select deps` 指定時）、パースできないファイルがあれば失敗。
- **高速** — 選択した解析のみ実行し、レポート生成をスキップします。

!!! note "パースエラーのカバー範囲"
    `complexity`、`deadcode`、`clones`、`mockdata`、`di` の各解析は読み込み・パースできなかったファイルを報告し、いずれか 1 つでも発生すればチェックは失敗します。`deps` は例外です: 依存グラフの構築処理は読めないモジュールを記録せずにスキップするため、`pyscn check --select deps` 単独では構文エラーを検出できません。デフォルト実行では `complexity` が常に走るためカバーされます。
...
| `--allow-parse-errors`   | off  | 読み込めない・パースできないファイルを警告のみとし、チェックを失敗させません。 |

デフォルトでは、パースできないファイルがあるとチェックは失敗します。そのようなファイルはすべての解析から除外されるため検出結果を一切生成せず、放置するとすべてのしきい値を通過してしまいます。つまり構文エラーを含むソースが「問題なし」と報告されることになります。この仕組みが届かない唯一の解析については上記の注記を参照してください。
...
| --- | --- |
| `0` | すべてのチェックに合格しました。 |
| `1` | 1 つ以上の品質しきい値を超過しました。 |
| `2` | 対象に対して解析を完了できませんでした（不正な入力、存在しないファイル、パースできないファイルなど）。 |

exit `1` はコードに対する判定であり、exit `2` は判定そのものが不完全であることを意味します。合格として扱わないでください。
