...
最初に見つかったファイルが使用されます。一致するファイルが見つかるか、ターゲットのリポジトリルート（`.git` を含むディレクトリ）に到達するか、ファイルシステムのルートに到達するまで親ディレクトリが検索されます。どちらのファイルも見つからない場合は、組み込みデフォルトが使用されます。

このリポジトリ境界により、別のワーキングツリー内にチェックアウトされたリポジトリへ無関係な設定が適用されるのを防ぎます。`vendor/other-repo/` を解析する場合、使用されるのは `vendor/other-repo/.pyscn.toml` か組み込みデフォルトであり、外側のリポジトリの設定は使用されません。`.git` ディレクトリを持たないリポジトリには停止すべき境界がないため、探索はファイルシステムのルートまで続きます。
