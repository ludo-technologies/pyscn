...
标记依赖了过多其他类的类（即 Chidamber & Kemerer 提出的对象间耦合度 CBO 指标）。pyscn 通过继承、直接实例化、对导入模块的属性访问和导入语句来统计一个类引用的不同类的数量。类型提示是声明而非使用，因此会显示在 `type_hint_dependencies` 明细中，但不计入分数。
