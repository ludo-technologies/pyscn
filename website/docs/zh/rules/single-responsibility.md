...
"关注点"由该模块依赖的名称推断而来：对于它导入的每个模块，分析器取依赖路径中与当前模块不重叠的第一个段，并跳过通用兜底名称（`base`、`common`、`helpers`、`node`、`shared`、`util`、`utils`）。去重后的段数即为 pyscn 归属于该模块的职责数量。导入它的模块（扇入）不计入关注点——被广泛复用本身不是一种职责。
