...
使用 `@staticmethod`、`@classmethod` 或 `@abstractmethod` 装饰的方法不引用实例状态，因此被排除在图之外。

构造函数（`__init__`、`__new__` 以及 dataclass 的 `__post_init__` 钩子）同样被排除。构造函数通常会初始化类的全部属性，若保留在图中，它建立的各个职责簇就会被连成一体，几乎任何类的 `LCOM4` 都会塌缩为 `1`。它引入的属性仍会计入 `instance_variables`。
