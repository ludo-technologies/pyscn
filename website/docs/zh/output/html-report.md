...
| 标签页 | 内容 |
| --- | --- |
| Functions | 函数复杂度指标、最复杂函数（前 20）、类执行作用域复杂度、最长函数、函数与类作用域中的死代码发现（前 20），以及两个折叠的可排序表格：所有模块和目录复杂度汇总。 |
...
内联脚本负责切换标签页（当前标签页会写入 URL 哈希，链接可直接打开指定标签页），并通过一个共享排序器处理模块和目录表格。没有网络请求。独立复杂度格式化器会在相同的目录指标下方，分别呈现函数和类作用域表格。
