...
- 通过时**退出码 0**，发现问题时**退出码 1**，分析无法完成时**退出码 2**。
- **严格的默认值** -- 复杂度超过 10 的函数即为失败；任何循环依赖即为失败（当设置了 `--select deps` 时）；任何无法解析的文件即为失败。
- **速度快** -- 仅运行你选择的分析；跳过报告生成。

!!! note "解析错误的覆盖范围"
    所有选定的分析都使用同一份项目发现记录。除非设置 `--allow-parse-errors`，解析错误会使检查失败，包括 `pyscn check --select deps`。文件读取错误始终会使检查失败。
...
| `--allow-parse-errors`   | 关闭  | 将解析错误视为警告。文件读取错误仍会使检查失败。 |

默认情况下，无法解析的文件会导致检查失败。这类文件被排除在所有分析之外，不会产生任何检查结果，因此会顺利通过所有阈值。`--allow-parse-errors` 不会忽略文件缺失、权限错误或其他读取错误。
...
| `0` | 所有检查通过。 |
| `1` | 一项或多项质量阈值被超出。 |
| `2` | 无法对指定目标完成分析（无效输入、文件缺失或无法解析的文件）。 |

退出码 `1` 是对代码的判定；退出码 `2` 意味着判定本身并不完整，不应视为通过。
