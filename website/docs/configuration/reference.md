...
`min_lines` and `min_nodes` have one exception: a function identical to another
function is always reported, however short it is, as long as its body has more
than one statement. Copy-pasted overrides across sibling classes are usually
shorter than the threshold, and dropping them would hide real duplication.
One-statement bodies (`pass`, `raise NotImplementedError`, a single `return`)
...
These keys only shape the report. They never change the duplication score or the health grade.
...
| `include_lazy_edges`   | bool   | `true`    | Include lazy / `TYPE_CHECKING` imports as graph edges. |
| `report_bridge_modules`| bool   | `true`    | Emit `bridge_modules` cross-community coupling list. |
| `resolution`           | float  | `1.0`     | Leiden resolution parameter (higher tends to produce more communities). |
...
See [Module Community Detection](../guides/module-community-detection.md) for interpretation, determinism guarantees, and JSON field reference.
