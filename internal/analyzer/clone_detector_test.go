...
func TestCloneDetector_ClassifyCloneType(t *testing.T) {
	...
	tests := []struct {
		...
		{"medium similarity", 0.75, Type4Clone},     // Changed: 0.75 is now Type4 (threshold 0.75)
		{"low similarity", 0.65, CloneType(0)},      // Changed: 0.65 is now not a clone (< 0.75)
...
func TestCloneDetector_IsSignificantClone(t *testing.T) {
	...
	tests := []struct {
		...
		{"threshold values", 0.60, 50.0, false}, // Changed: 0.60 is now below Type4 threshold (0.75)
	}
