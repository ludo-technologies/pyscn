...
func TestAPTEDAnalyzer_NilChecks(t *testing.T) {
	...
	t.Run("ComputeSimilarity with nil trees", func(t *testing.T) {
		// Both nil
		similarity := analyzer.ComputeSimilarityTrees(nil, nil)
		...
		// First nil
		similarity = analyzer.ComputeSimilarityTrees(nil, tree)
		assert.Equal(t, 0.0, similarity, "Similarity from nil to tree should be 0.0")

		// Second nil
		similarity = analyzer.ComputeSimilarityTrees(tree, nil)
