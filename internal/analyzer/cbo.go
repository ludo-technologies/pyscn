...
func (a *CBOAnalyzer) removeSameFileTopLevelPeers(dependencies *cboDependencies, ownerClass string, topLevelPeers map[string]bool) {
	...
	for _, bucket := range []map[string]bool{
		dependencies.all,
		dependencies.inheritance,
		dependencies.typeHints,
		dependencies.instantiations,
		dependencies.attributeAccesses,
		dependencies.imports,
	} {
		for depName := range bucket {
			if depName != ownerClass && topLevelPeers[depName] {
				delete(bucket, depName)
			}
		}
	}
}
...
func (a *CBOAnalyzer) addDependency(dependencies *cboDependencies, className string, kind cboDependencyKind) {
	...
	// A type annotation is a declaration, not a use: nothing in the class
	// breaks when the annotated type's internals change (Chidamber & Kemerer
	// count method/attribute usage only). Annotation references are kept in
	// the typeHints bucket for the metrics breakdown but never contribute to
	// CouplingCount or the risk level. See #757.
	if kind == dependencyKindTypeHint {
	...
	// Previously the import check returned early, which made the kind
