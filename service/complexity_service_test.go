...
`)
	...
	t.Run("parse errors still return raw metrics", func(t *testing.T) {
		...
		assert.Equal(t, filePath, response.Failures[0].FilePath)
		assert.Contains(t, response.Failures[0].Message, "Parse error")
		assert.Equal(t, fmt.Sprintf("[%s] %s", filePath, response.Failures[0].Message), response.Errors[0])
