		...
		err := filepath.Walk(root, func(path string, info os.FileInfo, err error) error {
			...
			for scanner.Scan() {
				...
				if matches := importRegex.FindStringSubmatch(line); len(matches) == 2 {
					...
					// Normalize to same relative base as relPath
					// e.g. both become "player.py" or "physics.py"
					if strings.Contains(importedRelPath, "/") {
						importedRelPath = filepath.Base(importedRelPath)
					}
